...
const RenderGridview = () => {
  ...
  const rowsPerPage = 12; // Made constant since it wasn't being changed
  ...
  const getDesignerData = async () => {
    ...
    try {
      const response = await axios.post(
        API_URL,
        { type: "render", page: currentPage, pageSize: rowsPerPage },
        {
          headers: {
  ...
  const debouncedSearch = useCallback(
    debounce(async (searchFilters) => {
      ...
      try {
        const response = await axios.post(
          SEARCH_API_URL,
          { type: "render", ...searchFilters },
          { headers: { Authorization: `Bearer ${savedToken}` } }
        );
        const data = response.data.data || [];
        setRows(data);
        setTotalRecords(data.length); // Using length since search might not return totalRecords
        setIsSearchActive(true);
      } catch (err) {
        setError(
          `Failed to search renders: ${
            err.response?.data?.message || err.message
          }`
        );
        if (
  ...
  useEffect(() => {
    ...
    if (!isSearchPending) {
      if (isSearchActive && (filters.status || filters.renderNo)) {
        ...
        <div className="">
          <div className="page-inner">
            <div className="d-flex justify-content-between align-items-center mb-3">
              <h3 className="fw-bold">Render Grid View</h3>
              <button
                      ...
                      <div className="row g-3">
                        ...
                        <div className="col-md-3">
                          ...
                          <button
                            ...
                            disabled={!filters.status && !filters.renderNo}
                          >
                            Clear Filters
                          </button>
                        </div>
                    ...
                    <div className="card-grid">
                      {rows.map((design, index) => (
                        <div className="card-item" key={index}>
                          ...
                          >
                            <Meta
                              title={`Render No: ${design.renderNo || "N/A"}`}
                            />
                            <Meta
                            ...
                            <Button
                              className={`antd_btn ${
                                design.renderStatus === "Approved"
                                  ? "btn-approved"
                                  : design.renderStatus === "Rejected"
                                  ? "btn-rejected"
                                  : "btn-pending"
                              }`}
                              style={{
