...
function GetMetal() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const goldRows = savedData.filter(item => item.metalTypeId === 3).map(item => ({
        ...
        const diamondRows = savedData.filter(item => item.metalTypeId === 2).map(item => ({
          id: item.id || item.itemId,
          Type: item.product_types || item.make_name || 'N/A',
        ...
        const colorstoneRows = savedData.filter(item => item.metalTypeId === 1).map(item => ({
          id: item.id || item.itemId,
          Type: item.product_types || item.make_name || 'N/A',
  ...
  const calculateTotalWeight = (data) => {
    return data.reduce((total, item) => total + Number(item.GrossWeight), 0).toFixed(4);
  };
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (response.status === 200 || response.data.success) {
            if (metalTypeId === 1) {
              setColorstoneData(prev => prev.filter(item => item.id !== itemId));
            } else if (metalTypeId === 2) {
              setDiamondData(prev => prev.filter(item => item.id !== itemId));
     ...
     <main className="main-content">
        ...
        <div className="">
          ...
          <div className="flex-grow-1 p-3">
            <center><h2>Metal Details</h2></center>
