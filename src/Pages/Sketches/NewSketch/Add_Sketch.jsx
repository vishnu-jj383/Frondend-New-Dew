...
function Add_Sketch() {
    ...
    const addSketch_Url = window.url + "sketch/addSketchFromDesign";
    ...
    useEffect(() => {
        ...
        const fetchDesigns = async () => {
            try {
                const savedToken = Cookies.get("authToken");
                const response = await axios.post(getDesign_Url, { customerId: customerId.value }, {
                    headers: { Authorization: `Bearer ${savedToken}`, "Content-Type": "application/json" }
                });
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            Swal.fire("Error", `Failed to add sketch: ${err.message}`, "error");
        }
    };

    const handleCloseModal = () => {
        setShowModal(false);
        navigate(-1); // Navigate back when modal is closed
    };
                        ...
                        {showModal && (
                            <div className="modal-overlay">
                                <div className="modal-content">
                                    <h2 className="modal-title">Add New Sketch</h2>
